...
export const mutationTypes = {
  ...
  searchProductStart: '[product] Search product start',
...
const mutations = {
  ...
  // SEARCH PRODUCT
  [mutationTypes.searchProductStart](state) {
    state.isLoading = true;
    state.searchResult = null;
  },
  [mutationTypes.searchProductSuccess](state, payload) {
    state.isLoading = false;
    state.searchResult = payload;
  },
  [mutationTypes.searchProductFailure](state, payload) {
    state.isLoading = false;
    state.validationErrors = payload;
  },
...
const actions = {
  ...
  [actionTypes.searchProduct]({ commit, dispatch }, payload) {
    return new Promise((resolve) => {
      commit(mutationTypes.searchProductStart);
      productApi
        .searchProduct(payload)
        .then((response) => {
          commit(mutationTypes.searchProductSuccess, response.data);
          resolve(response.data);
        })
        .catch((result) => {
          dispatch(actionTypesFromAuth.logout);
          commit(mutationTypes.searchProductFailure, result.response.data);
        });
    });
  },
